...
var getWind = function getWind(deg) {
  for (var i = 0; i < winds.length; i++) {
    if (deg > winds[i].deg - 5.625 && deg < winds[i].deg + 5.625) return "<abbr title=\"".concat(winds[i].name, "\">").concat(winds[i].abbr, "</abbr>");
  }
};
...
var getWeather = function getWeather() {
  return fetch("".concat(url, "weather?id=").concat(cityId, "&appid=").concat(appid, "&units=").concat(units, "&lang=").concat(lang)).then(function (resp) {
    ...
    document.querySelector('.card__icon').innerHTML = "<img src=\"".concat(weatherIcon, "@4x.png\">");
...
fetch(citiesList).then(function (resp) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (data[i]['name'] == 'London' || data[i]['name'] == 'Moscow' || data[i]['country'] == 'AZ' || data[i]['country'] == 'GE') {
      ...
      option.id = data[i]['id'];
      option.value = "".concat(data[i]['name'], ", ").concat(data[i]['country'], " (").concat(data[i]['id'], ")");
      option.textContent = "".concat(data[i]['name'], " - ").concat(data[i]['country']);
